        ...
        <ACalendar v-model:value="current" @panelChange="onPanelChange">
          <template #dateCellRender="{ current: day }">
            <div v-if="recOf(day)" class="rounded-lg border px-1.5 py-1 text-[11px] leading-tight"
              :class="cellBg[recOf(day).status] || 'bg-muted border-border'">
              <ATag :color="ATTENDANCE_STATUS[recOf(day).status]?.color" class="!mr-0 !mb-1">
              ...
              <div class="font-mono text-foreground/80">
                ...
                <div v-if="recOf(day).overtimeHours > 0" class="text-orange-600">TC: {{ recOf(day).overtimeHours }}h</div>
              </div>
            </div>
          </template>
        </ACalendar>

...
import { Select as ASelect, Table as ATable, Tag as ATag, message } from 'ant-design-vue'
...
// ===== Map tên nhân viên (giống PayrollsView) =====
...
// ===== Helper định dạng (cẩn thận null) =====
...
async function submitManual() {
  ...
  const ok = await store.upsertManual({
    ...
    status: form.status,
    checkInTime: form.checkInTime ? form.checkInTime : null,
    checkOutTime: form.checkOutTime ? form.checkOutTime : null,
    note: form.note || null,
  })
  ...
  if (ok) {
    ...
    message.error(store.error || 'Lưu chấm công thất bại')
  }
}
...
<template>
  <div class="space-y-8 motion-safe:animate-in motion-safe:fade-in duration-700 pb-12">
    ...
    <div class="bg-card border border-border rounded-2xl shadow-md p-6">
      <ATable :columns="columns" :data-source="store.attendanceList" :loading="store.isLoading" row-key="id" :pagination="{ pageSize: 10 }">
      ...
      <form class="space-y-5 font-sans" @submit.prevent="submitManual">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              type="date"
              class="w-full h-11 px-3 rounded-xl border border-border bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-accent"
            />
          </div>
          <div>
        ...
        <div>
          ...
          <textarea
            ...
            class="w-full px-3 py-2 rounded-xl border border-border bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-accent resize-none"
          ></textarea>
        </div>

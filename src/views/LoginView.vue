...
<template>
  <div class="min-h-screen flex items-center justify-center bg-background p-4 relative overflow-hidden">
    ...
    <div class="w-full max-w-md animate-in fade-in zoom-in-95 duration-1000">
      <div class="bg-card border border-border p-8 rounded-3xl shadow-2xl relative">
        <div class="text-center mb-8">
          ...
          <h1 class="font-display text-3xl mb-2 text-foreground">HR Core</h1>
        ...
        <form @submit.prevent="handleLogin" class="space-y-4">
          <div>
            <input v-model="username" type="text" placeholder="Username (admin, hr, manager, employee)" required class="w-full h-12 px-4 rounded-xl border border-border bg-transparent focus:ring-2 focus:ring-accent focus:ring-offset-2 outline-none transition-all font-mono text-sm" />
          </div>
          <div>
            <input v-model="password" type="password" placeholder="Password" required class="w-full h-12 px-4 rounded-xl border border-border bg-transparent focus:ring-2 focus:ring-accent focus:ring-offset-2 outline-none transition-all font-mono text-sm" />
          </div>
